...
\begin{figure}\centering
  \begin{tikzpicture}
    ...
    \draw[very thick, white, name path=to-be-green] (0, .6) to[out=0, in=180] (3, -.6);
...
\begin{definition}[Wirtinger presentation]
  Given an oriented diagram $D$ of knot $K$ with segments $a_1$, $a_2$, ..., $a_s$ and crossings $c_1$, ..., $c_x$ the knot group $\pi_1(K)$ can be represented as $\pi_1(K)=\langle G\;|\;R\rangle$, where $G$ is the set of segments of $D$ and relations $R$ correspond to crossings in the manner described in the diagram below
  \begin{center}
    \begin{tikzpicture}
      \begin{scope}
        \draw[->] (2, 0)--(0, 3);
    ...
    \end{tikzpicture}
  \end{center}
  Presentation $\langle G\;|\;R\rangle$ described above is called the \buff{Wirtinger presentation} \cite[Chapter~6]{livingstone}.
\end{definition}
...
\begin{definition}[Alexander module]\label{alexander module def}
  ...
  Given a group $G$ with a homomorphism $g:G\to \Z$, the abelianization of the commutator of a group $G$, $K_G^{ab}$, with $\Z[\Z]$-module structure is called the \buff{Alexander module} of $G$. If $G$ is a knot group, then it is the Alexander module of the knot $K$.
\end{definition}
