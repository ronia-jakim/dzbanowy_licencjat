...
$${ -t^{3}+5t^{2}-10t+13-10t^{-1}+5t^{-2}-t^{-3}} $$
  ...
  \begin{frame}{Poszukiwania niezmienników}
    W wyliczaniu wielomianu Alexandera tworzymy macierz $n\times n$, której kolumny odpowiadają segmentom, a wiersze skrzyżowaniom. Wielomian Alexandera to minory $(n-1)\times (n-1)$.
...
% \printbibliography
  ...
  %
  % \begin{frame}{Moduł Alexandera}
  %   Niech $X-S^3-K$. Oznaczmy $K_G=[\pi_1(X), \pi_1(X)]$.
  %   \begin{deff}{}
  %     \emph{Nieskończone cykliczne nakrycie} $X$ to przestrzeń ilorazowa
  %     $$\boldsymbol{\overline{X}=\widetilde{X}/K_G}.$$
  %   \end{deff}
  %   \begin{itemize}
  %     \item Na przestrzeni $\overline{X}$ działa pierścień $\Z[\Z]=\Z[t, t^{-1}]$ (powierzchnia Seiferta $K$).
  %     \item $H_1(\overline{X}, \Z)=K_G^{ab}$ ma strukturę $\Z[\Z]$-modułu i w dodatku jest modułem torsyjnym. Nazywamy go \textbf{modułem Alexandera}.
  %   \end{itemize}
  % \end{frame}
  %
  % \begin{frame}[fragile]{Moduł Alexandera}
  %   \begin{itemize}
  %     \item $H_1(\overline{X}, \Z)=K_G^{ab}=[\pi_1(X), \pi_1(X)]^{ab}$ ma strukturę $\Z[\Z]$-modułu, który nazywamy \textbf{modułem Alexandera}.
  %     \item Prezentacja grupy $K_G$ przychodzi z prezentacji Wirtingera grupy $\pi_1(X)$ - \emph{segmenty i skrzyżowania} diagramu $D$ węzła $K$.
  %   \end{itemize}
  %
  %   \begin{center}
  %     \begin{tikzcd}[column sep=small]
